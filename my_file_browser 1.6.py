...
class Sidebar(tk.Frame):
    ...
    def add_important_directories(self):
        ...
        important_dirs = {
            ...
            '垃圾箱': (os.path.join(os.path.expanduser('~'), '.trash'), 'trash')
        }
...
class FileBrowser(tk.Frame):
    ...
    def set_active(self, active=True):
        ...
        if active:
            # 激活状态：更深的蓝色背景
            self.configure(bg='#bbdefb')  # 更深的蓝色背景
            self.navbar.configure(bg='#bbdefb')
            self.file_frame.configure(bg='#bbdefb')
            self.statusbar.configure(bg='#bbdefb')
            
            # 添加边框高亮
            self.config(bd=2, relief=tk.RAISED)
            
            # 更改样式
            self.style.configure('TButton', background='#bbdefb')
            self.style.configure('Treeview.Heading', background='#90caf9')  # 更深的表头背景
        else:
            # 非激活状态：默认背景色
            self.configure(bg='#f2f2f7')
    ...
    def on_delete_press(self, event):
        ...
        if selected:
            ...
            # 根据是否为非空目录显示不同的确认消息
            if is_non_empty_dir:
                confirm_msg = f"确定要将目录 '{display_name}' 及其所有内容移动到垃圾箱吗？"
            else:
                confirm_msg = f"确定要将 {display_name} 移动到垃圾箱吗？"
            
            if messagebox.askyesno('移动到垃圾箱', confirm_msg):
                try:
                    # 通过app的_move_to_trash方法移动到垃圾箱
                    if hasattr(self, 'app') and hasattr(self.app, '_move_to_trash'):
                        if self.app._move_to_trash(path):
                            self.refresh_file_list()
                            messagebox.showinfo('成功', f'已成功将 {display_name} 移动到垃圾箱')
                        else:
                            messagebox.showerror('操作失败', f'无法将 {display_name} 移动到垃圾箱')
                    else:
                        # 如果无法访问app，使用默认的删除方式
                        if os.path.isdir(path):
                            shutil.rmtree(path)
...
class FinderBrowser(tk.Tk):
    ...
    def __init__(self):
        ...
        # 配置文件路径
        self.config_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'my_file_browser.ini')
        ...
        # 设置窗口位置（如果配置中没有保存的位置，则居中显示）
        if 'window_geometry' not in self.app_config:
            self.center_window()
    ...
    def save_config(self):
        ...
        try:
            ...
            # 更新浏览器路径
            if hasattr(self, 'left_browser') and hasattr(self.left_browser, 'current_path'):
                self.app_config['left_browser_path'] = self.left_browser.current_path
            
            if hasattr(self, 'right_browser') and hasattr(self.right_browser, 'current_path'):
                self.app_config['right_browser_path'] = self.right_browser.current_path
    ...
    def _move_to_trash(self, path):
        ...
        try:
            ...
            while os.path.exists(trash_dest):
                trash_name = f"{name}_{timestamp}_{counter}"
                trash_dest = os.path.join(self.trash_path, trash_name)
                counter += 1
    ...
    def empty_trash(self):
        ...
        if not os.path.exists(self.trash_path):
            messagebox.showinfo("提示", "垃圾箱已经是空的")
            return
        ...
        # 确认清空垃圾箱
        confirm_msg = f"确定要永久删除垃圾箱中的 {len(items)} 个项目吗？此操作不可撤销！"
        if messagebox.askyesno("确认清空垃圾箱", confirm_msg):
            try:
                for item in items:
                    ...
                    if os.path.isdir(item_path):
                        ...
                        os.remove(item_path)
                messagebox.showinfo("成功", f"已清空垃圾箱，删除了 {len(items)} 个项目")
            except Exception as e:
                messagebox.showerror("错误", f"清空垃圾箱失败: {e}")
    ...
    def on_closing(self):
        ...
        # 关闭所有SFTP连接
        if hasattr(self, 'left_browser') and hasattr(self.left_browser, 'sftp_fs'):
            try:
                self.left_browser.sftp_fs.close()
            except:
                pass
        
        if hasattr(self, 'right_browser') and hasattr(self.right_browser, 'sftp_fs'):
            try:
                self.right_browser.sftp_fs.close()
            except:
                pass
    ...
    def create_menu(self):
        ...
        self.file_menu.add_command(label='清空垃圾箱', command=self.empty_trash)
        self.file_menu.add_separator()
        ...
        self.edit_menu.add_command(label='删除到垃圾箱', command=self.delete)
    ...
    def create_main_content(self):
        ...
        # 使用保存的左侧浏览器路径或默认路径
        left_initial_path = self.app_config.get('left_browser_path', '/')
        ...
        # 导航到保存的路径
        self.left_browser.navigate_to(left_initial_path)
        ...
        # 使用保存的右侧浏览器路径或默认路径
        right_initial_path = self.app_config.get('right_browser_path', '/')
        ...
        # 导航到保存的路径
        self.right_browser.navigate_to(right_initial_path)
    ...
    def on_sidebar_path_select(self, path):
        ...
        if active_browser:
            ...
            if hasattr(active_browser, 'is_sftp') and active_browser.is_sftp:
                # 在SFTP模式下，直接断开SFTP连接，然后导航到本地路径
                self._disconnect_sftp_internal()
                # 导航到选中的本地路径
    ...
    class MoveProgressDialog(tk.Toplevel):
        """移动进度对话框"""
        def __init__(self, parent, title="移动文件"):
            super().__init__(parent)
            self.title(title)
            self.transient(parent)
            self.resizable(False, False)
            ...
            # 进度条标签
            self.progress_label = ttk.Label(self.progress_frame, text="正在准备移动...")
            self.progress_label.pack(fill=tk.X, pady=(0, 10))
        ...
        def update_progress(self, progress, current_file=""):
            ...
            self.progress_var.set(progress)
            if current_file:
                self.progress_label.config(text=f"正在移动: {os.path.basename(current_file)}")
            self.update_idletasks()
            
        def cancel(self):
            """取消移动操作"""
            self.cancelled = True
            self.progress_label.config(text="正在取消移动...")
            self.cancel_btn.config(state=tk.DISABLED)
    ...
    def delete(self):
        ...
        # 检查是否为SFTP浏览器
        if hasattr(active_browser, 'is_sftp') and active_browser.is_sftp:
            # 如果是SFTP浏览器，调用SFTP多选删除方法（SFTP仍然直接删除）
        ...
        if len(items_to_delete) == 1:
            ...
            if has_non_empty_dir:
                confirm_msg = f"确定要将选中的 {len(items_to_delete)} 个项目（包含非空目录）移动到垃圾箱吗？"
            else:
                confirm_msg = f"确定要将选中的 {len(items_to_delete)} 个项目移动到垃圾箱吗？"
        
        # 确认删除
        response = messagebox.askyesno("移动到垃圾箱", confirm_msg)
        if response:
            success_count = 0
            error_files = []
            ...
            for path, display_name, is_dir in items_to_delete:
                try:
                    ...
                    if self._move_to_trash(path):
                        ...
                        error_files.append(f"{display_name}: 移动到垃圾箱失败")
                except Exception as e:
            ...
            # 显示结果
            if error_files:
                error_msg = f"已移动到垃圾箱 {success_count} 个项目，{len(error_files)} 个失败:\n" + "\n".join(error_files)
                messagebox.showerror("操作结果", error_msg)
                self.status_var.set(f"已移动到垃圾箱 {success_count} 个项目，{len(error_files)} 个失败")
            else:
                messagebox.showinfo("成功", f"已成功将 {success_count} 个项目移动到垃圾箱")
                self.status_var.set(f"已移动到垃圾箱 {success_count} 个项目")
    ...
    def navigate_to_home(self):
        ...
        if active_browser:
            # 检查是否为SFTP模式
            if hasattr(active_browser, 'is_sftp') and active_browser.is_sftp:
                messagebox.showwarning("操作不支持", "SFTP模式下无法导航到本地主文件夹")
                return
    ...
    def navigate_to_desktop(self):
        ...
        if active_browser:
            # 检查是否为SFTP模式
            if hasattr(active_browser, 'is_sftp') and active_browser.is_sftp:
                messagebox.showwarning("操作不支持", "SFTP模式下无法导航到本地桌面目录")
                return
    ...
    def navigate_to_documents(self):
        ...
        if active_browser:
            # 检查是否为SFTP模式
            if hasattr(active_browser, 'is_sftp') and active_browser.is_sftp:
                messagebox.showwarning("操作不支持", "SFTP模式下无法导航到本地文档目录")
                return
    ...
    def navigate_to_downloads(self):
        ...
        if active_browser:
            # 检查是否为SFTP模式
            if hasattr(active_browser, 'is_sftp') and active_browser.is_sftp:
                messagebox.showwarning("操作不支持", "SFTP模式下无法导航到本地下载目录")
                return
    ...
    def navigate_to_music(self):
        ...
        if active_browser:
            # 检查是否为SFTP模式
            if hasattr(active_browser, 'is_sftp') and active_browser.is_sftp:
                messagebox.showwarning("操作不支持", "SFTP模式下无法导航到本地音乐目录")
                return
    ...
    def navigate_to_pictures(self):
        ...
        if active_browser:
            # 检查是否为SFTP模式
            if hasattr(active_browser, 'is_sftp') and active_browser.is_sftp:
                messagebox.showwarning("操作不支持", "SFTP模式下无法导航到本地图片目录")
                return
    ...
    def navigate_to_movies(self):
        ...
        if active_browser:
            # 检查是否为SFTP模式
            if hasattr(active_browser, 'is_sftp') and active_browser.is_sftp:
                messagebox.showwarning("操作不支持", "SFTP模式下无法导航到本地影片目录")
                return
    ...
    def set_active_browser(self, browser):
        ...
        if browser == self.left_browser:
            ...
            # 同时更新外层框架的背景色
            self.left_frame.configure(bg='#bbdefb')
            ...
            self.right_frame.configure(bg='#bbdefb')
    ...
    def create_toolbar(self):
        ...
        # SFTP连接/断开按钮（动态切换）
        self.sftp_btn = ttk.Button(self.toolbar, text='SFTP连接', command=self.toggle_sftp_connection)
        self.sftp_btn.pack(side=tk.LEFT, padx=5, pady=5)
        
        # 添加弹性空间，将关于按钮推到最右侧
        ttk.Frame(self.toolbar).pack(side=tk.LEFT, fill=tk.X, expand=True)
        
        # 关于按钮
        self.about_btn = ttk.Button(self.toolbar, text='关于', command=self.show_about_info)
            ...
            try:
                ...
                for source_browser, target_browser, source_path, target_path, display_name in files_to_copy:
                    ...
                    if is_source_sftp and is_target_sftp:
                        ...
                        if os.path.isdir(source_path):
                            # 递归上传目录
                            def sftp_upload_recursive(sftp_fs, source_path, target_path):
                                # 创建目标目录
                                if not sftp_fs.exists(target_path):
                                    sftp_fs.mkdir(target_path)
                                ...
                                for item in os.listdir(source_path):
                                    ...
                                    if os.path.isdir(item_source_path):
                                        # 递归上传子目录
                                        sftp_upload_recursive(sftp_fs, item_source_path, item_target_path)
                                    else:
                                        # 上传单个文件
                                        sftp_fs.put(item_source_path, item_target_path)
                                        # 更新已复制大小
                            ...
                            # 开始递归上传
                            sftp_upload_recursive(target_browser.sftp_fs, source_path, target_path)
                        else:
                            # 上传单个本地文件
                            # 确保目标目录存在
    ...
    def _move_with_progress(self, source_path, target_path, total_size, copied_size, progress_dialog, is_source_sftp, is_target_sftp, active_browser, target_browser):
        """带进度条的移动操作"""
        # 检查是否被取消
        if progress_dialog.cancelled:
            raise OSError("移动操作已取消")
        ...
        if is_source_sftp and is_target_sftp:
            ...
            # SFTP到本地移动（下载）
            if active_browser.sftp_fs.isdir(source_path):
                # 递归下载目录
                def sftp_download_recursive(sftp_fs, remote_path, local_path):
                    import stat
                    # 创建本地目录
                    os.makedirs(local_path, exist_ok=True)
                    
                    # 遍历远程目录
                    for item in sftp_fs.sftp.listdir_attr(remote_path):
                        item_name = item.filename
                        remote_item_path = sftp_fs.join(remote_path, item_name)
                        ...
                        if stat.S_ISDIR(item.st_mode):
                            # 递归处理子目录
                            sftp_download_recursive(sftp_fs, remote_item_path, local_item_path)
                        else:
                            # 下载文件
                            sftp_fs.get(remote_item_path, local_item_path)
                            # 更新已移动大小
                            file_size = item.st_size
                            copied_size[0] += file_size
                            # 更新进度显示
                            progress_dialog.update_progress(copied_size[0] / total_size * 100 if total_size > 0 else 0, remote_item_path)
                
                # 执行递归下载
                sftp_download_recursive(active_browser.sftp_fs, source_path, target_path)
                ...
                sftp_recursive_delete(active_browser.sftp_fs.sftp, source_path)
            else:
                # 下载单个文件
                # 获取文件大小
                file_stat = active_browser.sftp_fs.sftp.stat(source_path)
            ...
            if os.path.isdir(source_path):
                # 递归上传目录
                def local_upload_recursive(sftp_fs, local_path, remote_path):
                    # 创建远程目录
                    sftp_fs.mkdir(remote_path)
                    ...
                    for item_name in os.listdir(local_path):
                        ...
                        if os.path.isdir(local_item_path):
                            # 递归处理子目录
                            local_upload_recursive(sftp_fs, local_item_path, remote_item_path)
                        else:
                            # 上传文件
                            sftp_fs.put(local_item_path, remote_item_path)
                            # 更新已移动大小
                            file_size = os.path.getsize(local_item_path)
                            copied_size[0] += file_size
                            # 更新进度显示
                            progress_dialog.update_progress(copied_size[0] / total_size * 100 if total_size > 0 else 0, local_item_path)
                
                # 执行递归上传
                local_upload_recursive(target_browser.sftp_fs, source_path, target_path)
            ...
            if os.path.isdir(source_path):
                ...
                # 更新已移动大小
                dir_size = get_dir_size(target_path)  # 移动后计算大小
                copied_size[0] += dir_size
                # 更新进度显示
                progress_dialog.update_progress(copied_size[0] / total_size * 100 if total_size > 0 else 0, source_path)
            else:
                # 移动单个文件
    ...
    def _move_with_dialog(self, files_to_move, active_browser, target_browser):
        """带进度条对话框的移动操作"""
        # 计算总大小
        total_size = 0
        for source_path, _, _, is_source_sftp, _ in files_to_move:
            try:
                if is_source_sftp:
                    if active_browser.sftp_fs.isdir(source_path):
                        # 计算SFTP目录大小
                        def get_sftp_dir_size(sftp, path):
                            size = 0
                            import stat
                            for item in sftp.listdir_attr(path):
                                item_path = f"{path}/{item.filename}"
                                if stat.S_ISDIR(item.st_mode):
                                    size += get_sftp_dir_size(sftp, item_path)
                                else:
                                    size += item.st_size
                            return size
                        total_size += get_sftp_dir_size(active_browser.sftp_fs.sftp, source_path)
                    else:
                        # 获取SFTP文件大小
                        file_stat = active_browser.sftp_fs.sftp.stat(source_path)
                    ...
                    if os.path.isdir(source_path):
                        ...
                        def get_local_dir_size(path):
                            ...
                            return size
                        total_size += get_local_dir_size(source_path)
                    else:
                        # 获取本地文件大小
                        total_size += os.path.getsize(source_path)
            except Exception as e:
                print(f"[ERROR] 计算文件大小失败: {str(e)}")
        
        # 创建进度条对话框
        progress_dialog = self.MoveProgressDialog(self, "移动文件")
        ...
        def move_task():
            ...
            try:
                for source_path, target_path, display_name, is_source_sftp, is_target_sftp in files_to_move:
                    ...
                    # 执行移动
                    try:
                        self._move_with_progress(source_path, target_path, total_size, copied_size, progress_dialog, is_source_sftp, is_target_sftp, active_browser, target_browser)
                        success_count += 1
                ...
                if not progress_dialog.cancelled:
                    self.status_var.set(f"移动失败: {str(e)}")
                    messagebox.showerror("错误", f"移动失败:\n{str(e)}")
            finally:
                # 关闭进度条对话框
                progress_dialog.destroy()
    ...
    def move(self):
        ...
        # 使用带进度条的移动方法
        self._move_with_dialog(files_to_move, active_browser, target_browser)
    ...
    class SftpLoginDialog(tk.Toplevel):
        ...
        def __init__(self, parent):
            ...
            # 获取保存的SFTP信息
            saved_sftp_info = parent.app_config.get('sftp_info', {})
            ...
            self.host_var = tk.StringVar(value=saved_sftp_info.get('host', ''))
            ...
            self.port_var = tk.StringVar(value=saved_sftp_info.get('port', '22'))
            ...
            self.username_var = tk.StringVar(value=saved_sftp_info.get('username', ''))
            ...
            self.password_var = tk.StringVar(value=saved_sftp_info.get('password', ''))
            ...
            self.path_var = tk.StringVar(value=saved_sftp_info.get('path', '/'))
    ...
    def connect_to_sftp(self, browser, sftp_info):
        ...
        try:
            ...
            # 保存SFTP登录信息到配置
            self.app_config['sftp_info'] = {
                'host': sftp_info['host'],
                'port': str(sftp_info['port']),  # 转换为字符串保存
                'username': sftp_info['username'],
                'password': sftp_info['password'],
                'path': sftp_info['path']
            }
    ...
    def toggle_sftp_connection(self):
        ...
        if hasattr(active_browser, 'is_sftp') and active_browser.is_sftp:
            ...
            # 当前是本地模式，执行连接
            self.open_sftp_dialog()
    
    def _disconnect_sftp_internal(self):
        """内部断开SFTP连接方法"""
        # 获取当前活动浏览器
        active_browser = self.get_active_browser()
        if not active_browser:
        ...
        # 检查是否为SFTP模式
        if not hasattr(active_browser, 'is_sftp') or not active_browser.is_sftp:
            return
        
        try:
            # 关闭SFTP连接
            if hasattr(active_browser, 'sftp_fs'):
                active_browser.sftp_fs.close()
            
            # 恢复原始方法
            if hasattr(active_browser, '_original_refresh'):
                active_browser.refresh_file_list = active_browser._original_refresh
                active_browser.open_file = active_browser._original_open_file
                active_browser.on_delete_press = active_browser._original_delete_press
                if hasattr(active_browser, '_original_sort_by_column') and active_browser._original_sort_by_column:
                    active_browser.sort_by_column = active_browser._original_sort_by_column
            
            # 重置SFTP标志
            active_browser.is_sftp = False
            
            # 清理SFTP相关属性
            if hasattr(active_browser, 'sftp_fs'):
                delattr(active_browser, 'sftp_fs')
            
            # 重置当前路径到用户主目录
            active_browser.current_path = os.path.expanduser('~')
            
            # 重置历史记录
            if hasattr(active_browser, 'history'):
                active_browser.history = []
                active_browser.history_index = -1
            
            # 刷新文件列表
            active_browser.refresh_file_list()
            
            # 更新按钮文本为"SFTP连接"
            self.sftp_btn['text'] = 'SFTP连接'
            
            # 更新状态栏
            self.status_var.set("已断开SFTP连接，返回本地模式")
            
        except Exception as e:
            error_msg = f"断开SFTP连接失败: {str(e)}"
            messagebox.showerror("错误", error_msg)
            self.status_var.set("断开连接失败")
    ...
    def show_about(self):
        """显示关于对话框"""
        messagebox.showinfo('关于', 'my_file_browser - 双窗口文件浏览器\n\n版本: 1.6\n模仿macOS Finder的双窗口文件浏览器')
    ...
    def show_about_info(self):
        ...
        y = (self.winfo_height() // 2) - (200 // 2) + self.winfo_y()
        ...
        # 创建主框架
        main_frame = tk.Frame(about_window, bg='#f8f9fa', padx=20, pady=20)
        main_frame.pack(fill=tk.BOTH, expand=True)
        
        # 程序名称（大字体，加粗）
        title_label = tk.Label(main_frame, text="my_file_browser", 
                              font=('Helvetica', 18, 'bold'), 
                              fg='#2c3e50', bg='#f8f9fa')
        title_label.pack(pady=(10, 5))
        
        # 版本信息
        version_label = tk.Label(main_frame, text="版本 1.6", 
                                font=('Helvetica', 12), 
                                fg='#7f8c8d', bg='#f8f9fa')
        ...
        # 创建者信息
        creator_label = tk.Label(main_frame, text="Created by Shylock Wolf", 
                               font=('Helvetica', 11), 
                               fg='#34495e', bg='#f8f9fa')
        creator_label.pack(pady=(5, 2))
        
        # 日期信息
        date_label = tk.Label(main_frame, text="2025/10/27", 
                             font=('Helvetica', 10), 
                             fg='#95a5a6', bg='#f8f9fa')
        date_label.pack(pady=(2, 15))
